...
export default function StatsPage() {
  ...
  // Projections (moyenne journalière × jours restants)
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
        ...
        <div className="max-w-2xl mx-auto px-4">
          <div className="flex items-center justify-between h-14">
            <div className="flex items-center space-x-2">
              ...
              <h1 className="text-lg font-semibold text-gray-900">Journal & Stats</h1>
            </div>
            <div className="flex items-center space-x-1">
              <button onClick={() => setMoisActuel(new Date(moisActuel.getFullYear(), moisActuel.getMonth() - 1, 1))} className="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-xl touch-manipulation">
                <ChevronLeft className="h-5 w-5" />
              </button>
              <span className="text-sm font-medium text-gray-900 capitalize px-2">{moisLabel}</span>
              <button onClick={() => setMoisActuel(new Date(moisActuel.getFullYear(), moisActuel.getMonth() + 1, 1))} className="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-xl touch-manipulation">
      ...
      <main className="max-w-2xl mx-auto px-4 py-5 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <StatCard icon={<ArrowUpRight className="h-5 w-5" />} label="Achats" value={formatPrice(achats)} color="text-red-600" bg="bg-red-50" />
          <StatCard icon={<ArrowDownRight className="h-5 w-5" />} label="CA" value={formatPrice(ca)} color="text-green-600" bg="bg-green-50" />
          <StatCard icon={<TrendingUp className="h-5 w-5" />} label="Bénéfice" value={formatPrice(benefice)} color="text-blue-600" bg="bg-blue-50" />
          <StatCard icon={<Package className="h-5 w-5" />} label="Vendus" value={`${nbArticlesVendus}`} color="text-purple-600" bg="bg-purple-50" />
        </div>
        ...
        {topArticles.length > 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 p-4">
            ...
            <div className="space-y-2">
              {topArticles.map((a, i) => (
                <div key={a.nom} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="font-medium text-sm text-gray-900">{a.nom}</p>
                      <p className="text-xs text-gray-500">{a.quantite} {a.unite} • {formatPrice(a.ca)}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-semibold text-green-600">{formatPrice(a.benefice)}</p>
        ...
        <div className="flex items-center justify-between">
          <div className="flex space-x-2 overflow-x-auto pb-1 -mx-4 px-4 scrollbar-hide">
            ...
            <FilterButton label="Entrées" count={entrees.length} active={filter === 'ENTREE'} onClick={() => setFilter('ENTREE')} color="bg-emerald-50 text-emerald-700" activeColor="bg-emerald-600 text-white" />
            <FilterButton label="Sorties" count={sorties.length} active={filter === 'SORTIE'} onClick={() => setFilter('SORTIE')} color="bg-red-50 text-red-700" activeColor="bg-red-600 text-white" />
          </div>
          <button className="px-4 py-2 bg-gray-100 text-gray-700 rounded-xl text-sm font-medium flex items-center space-x-1 hover:bg-gray-200 touch-manipulation">
          ...
          {loading ? (
            ...
            <div className="divide-y divide-gray-50">
              {mvtDuMois.slice(0, 50).map(m => (
                <div key={m.id} className="p-4 flex items-center justify-between hover:bg-gray-50 active:bg-gray-100 touch-manipulation">
                  <div className="flex items-center space-x-3">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${m.type === 'ENTREE' ? 'bg-emerald-100' : 'bg-red-100'}`}>
                      {m.type === 'ENTREE' ? (
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 flex items-center space-x-1">
                        ...
                        {m.note && <span className="text-gray-400">·</span>}
  ...
  return (
    <button
      ...
      className={`flex items-center space-x-1.5 px-4 py-2 rounded-xl text-sm font-medium flex-shrink-0 touch-manipulation transition-all ${
        active ? (activeColor || 'bg-gray-900 text-white') : (color || 'bg-gray-100 text-gray-700')
      }`}
    >
      <span>{label}</span>
